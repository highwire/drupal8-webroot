...
class ContentEntity extends DatasourcePluginBase implements EntityDatasourceInterface, PluginFormInterface {
  ...
  public function getPropertyDefinitions() {
    ...
    // Some properties can't be indexed because we can't extract them
    // currently, due to a shortcoming of Core's Typed Data API. See #2695527
    $excluded_properties = ['path'];
    foreach ($excluded_properties as $excluded_property) {
      if (!empty($properties[$excluded_property])) {
        unset($properties[$excluded_property]);
      }
    }
