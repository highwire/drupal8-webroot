<?php

namespace HighWire\Clients\CasaToken;

use HighWire\Clients\Client;
use GuzzleHttp\Client as GuzzleClient;
use CasaTokenClient\Api\ValidationApi;
use CasaTokenClient\Configuration;


/**
 * Casa Validation Client Class.
 */
class Validation extends Client {

  /**
   * Transfer Validation swagger client.
   *
   * @var \CasaTokenClient\Api\ValidationApi
   */
  protected $validationSwaggerClient;

  /**
   * Client context.
   *
   * @var string
   */
  protected $context;

  /**
   * Create a new client object.
   *
   * @param \GuzzleHttp\Client $http_client
   *   A guzzle client object.
   * @param array $config
   *   Any configuration the client needs.
   */
  public function __construct(GuzzleClient $http_client, array $config = []) {
    parent::__construct($http_client, $config);
    $swagger_config = new Configuration();
    $swagger_config->setHost($http_client->getConfig('base_uri')->__toString());
    if (isset($config['context'])) {
      $this->context = $config['context'];
    }
    $this->validationSwaggerClient = new ValidationApi($this->httpClient, $swagger_config);
  }

  /**
   * Implements php's _call method to make writing clients easier.
   *
   * @param string $name
   *   The name of the method being called.
   * @param array $arguments
   *   Any arguments passed to the method.
   *
   * @return mixed
   *   A response from the client or what ever the return
   *   is from the magic method that was invoked.
   */
  public function __call($name, array $arguments) {
    if (method_exists($this->validationSwaggerClient, $name)) {
      return call_user_func_array([$this->validationSwaggerClient, $name], $arguments);
    }
    elseif (!method_exists($this, $name)) {
      throw new \Exception("Method doesn't exist $name");
    }
  }

  /**
   * Get the client.
   *
   * @return \CasaTokenClient\Api\ValidationApi
   *   A swagger validation api client.
   */
  public function getClient(array $config = []) {
    return $this->validationSwaggerClient;
  }

  /**
   * Get the client context.
   *
   * @return string
   *   The client context setting.
   */
  public function getContext() {
    return $this->context;
  }

}
