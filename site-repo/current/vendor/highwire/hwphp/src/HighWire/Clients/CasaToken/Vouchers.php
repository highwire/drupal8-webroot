...
use CasaTokenClient\Api\VouchersApi;
use CasaTokenClient\Configuration;


/**
 * Casa Vouchers Client Class.
 */
class Vouchers extends Client {

  /**
   * Transfer Vouchers swagger client.
   *
   * @var \CasaTokenClient\Api\VouchersApi
   */
  protected $vouchersSwaggerClient;
  ...
  public function __construct(GuzzleClient $http_client, array $config = []) {
    ...
    $this->vouchersSwaggerClient = new VouchersApi($this->httpClient, $swagger_config);
  }
  ...
  public function __call($name, array $arguments) {
    if (method_exists($this->vouchersSwaggerClient, $name)) {
      return call_user_func_array([$this->vouchersSwaggerClient, $name], $arguments);
    }
   ...
   * Get the client.
   *
   * @return \CasaTokenClient\Api\VouchersApi
   *   A swagger vouchers api client.
   */
  public function getClient() {
    return $this->vouchersSwaggerClient;
  }
