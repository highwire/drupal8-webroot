...
use PersonalizationClient\Api\AlertsApi;
...
class Alerts extends Client {
   ...
   *
   * @var \PersonalizationClient\Api\AlertsApi
   */
  protected $alertsSwaggerClient;
  ...
  public function __construct(GuzzleClient $http_client, array $config = []) {
    ...
    $this->alertsSwaggerClient = new AlertsApi($this->httpClient, $swagger_config);
  }
  ...
  public function __call($name, array $arguments) {
    if (method_exists($this->alertsSwaggerClient, $name)) {
      return call_user_func_array([$this->alertsSwaggerClient, $name], $arguments);
    }
